    ...
    *,
    ...
    try:
        if collection_id is None:
            collection = db.query(CollectionModel).filter(CollectionModel.name == "default").first()
            if not collection:
                collection = CollectionModel(id=str(uuid.uuid4()), name="default")
                db.add(collection)
                db.commit()
...
async def _process_files(
    ...
    db: Session = None,
) -> Tuple[List[UploadedDocument], List[dict]]:
    """
    Common function to process a list of files.
    Returns (uploaded_files, failed_files)
    """
    uploaded_files: List[UploadedDocument] = []
    failed_files: List[dict] = []

    slots_left = float('inf')  # unlimited
    if check_limits and collection_id:
        existing_docs_count = (
    ...
    for file in files:
        ...
        if check_limits and slots_left <= 0:
            failed_files.append({
                "filename": file.filename,
...
async def upload_files_to_collection(
    ...
    uploaded_files, failed_files = await _process_files(files, collection_id, check_limits=True, db=db)
...
async def upload_files(
    ...
    **Respuesta:**
    ...
    uploaded_files, failed_files = await _process_files(files, collection_id=None, check_limits=False, db=db)
