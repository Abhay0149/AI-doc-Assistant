...
from app.db.models import Chunk, ChatMessage, Session as SessionModel, Collection as CollectionModel
...
async def ask_question(request: QuestionRequest, db: Session = Depends(get_db)):
    ...
    **Respuesta:** Siempre incluye `session_id` (creado por el backend si no lo enviaste), además de `question`, `answer`, `results`, `context_used`.
    ...
    # Set collection_id to default if not provided
    ...
    if db.get(SessionModel, effective_session_id) is None:
        db.add(SessionModel(id=effective_session_id, collection_id=collection_id))
        db.flush()
