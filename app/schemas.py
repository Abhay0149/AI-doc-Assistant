...
class ChatMessage(BaseModel):
    ...
    """
    id: Optional[str] = Field(default=None, description="UUID del mensaje en BD. En el payload debe ser null u omitido; solo viene en la respuesta.")
    role: str = Field(..., description="'user' o 'assistant'.")
    content: str = Field(..., description="Contenido del mensaje.")
    created_at: Optional[str] = Field(default=None, description="Fecha del mensaje en ISO 8601. Solo en la respuesta.")

    @field_validator("id")
    @classmethod
    def id_empty_as_none(cls, v: Optional[str]) -> Optional[str]:
...
class QuestionRequest(BaseModel):
    ...
    collection_id: Optional[str] = Field(
        default=None,
        description="ID de la colección. Si no se envía, se usa la colección por defecto.",
    )

class CollectionCreate(BaseModel):
    """Cuerpo de la petición para el endpoint /api/collections."""
    name: str = Field(..., description="Nombre de la colección.")
    description: Optional[str] = Field(default=None, description="Descripción de la colección.")
    is_public: bool = Field(..., description="Indica si la colección es pública.")
    code: Optional[str] = Field(default=None, description="Código de acceso; obligatorio si is_public es False.")
...
class CollectionResponse(BaseModel):
    ...
    description: Optional[str] = Field(default=None, description="Descripción de la colección.")
    is_public: bool = Field(..., description="Indica si la colección es pública.")
    code: Optional[str] = Field(default=None, description="Código de acceso a la colección.")
    created_at: Optional[str] = Field(None, description="Fecha de creación de la colección (ISO 8601).")
    updated_at: Optional[str] = Field(None, description="Fecha de última actualización de la colección (ISO 8601).")
...
class DocumentListItem(BaseModel):
    ...
    created_at: Optional[str] = Field(None, description="Fecha de subida (ISO 8601).")
...
class SessionResponse(BaseModel):
    ...
    created_at: Optional[str] = Field(None, description="Fecha de creación de la sesión (ISO 8601).")
